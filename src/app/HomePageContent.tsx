...
export default function HomePageContent() {
  ...
  useEffect(() => {
    ...
    const itemId = searchParams.get('id'); // Get ID for potential direct navigation
    if (view && ['tasks', 'goals', 'notes', 'calendar'].includes(view)) {
      setViewMode(view);
      // If an ID is present, you might want to scroll to it or highlight it in the respective view
      // This logic would be specific to each view component.
      if (itemId) {
        console.log(`Navigating to view: ${view}, item ID: ${itemId}`);
      }
    } else {
      setViewMode('dashboard'); // Default to dashboard if no valid view param
    }
  }, [searchParams]);
  ...
  const onCategoryChange = (category: Category) => {
    ...
    if (isAiChatModeActive) {
       setAiChatHistory([]);
       sendAiRequest([{text: `Switched to ${category} project view.`}], [], 'chatSession'); 
    }
    setAiMessageForCollapsedWidget(null); 
  };
