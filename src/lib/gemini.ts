...
const generationConfig = {
  temperature: 0.7, // Balanced for conversational creativity and instruction following
  topP: 0.95,
  topK: 60,
...
export interface AiOperation {
  action: "addTask" | "addNote" | "addGoal" | "addEvent" | "updateTask" | "unknown" ;
...
export async function processWithGemini(
    inputParts: Part[], // Will be empty for initial chat greeting
  ...
  **YOU MUST TREAT THIS AS A CONTINUOUS MEMORY.** This is not optional trivia — this is your working memory of the user.
  ...
  // If inputParts is empty (e.g., initial chat greeting), send a minimal user part to trigger a response.
  // Otherwise, use the provided inputParts.
  const currentInputParts = (inputParts && inputParts.length > 0) ? inputParts : [{ text: "Hello AIDA." }]; // Default trigger for initial greeting
  contents.push({ role: "user", parts: currentInputParts });
  ...
  try {
    ...
    return { 
        ...
        originalInputParts: inputParts, // Return original input parts for context if needed by caller
    };
...
`
    ...
    const systemInstructionText = promptForSummary;
